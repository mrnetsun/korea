	...
	*/
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			$this->response(array('status' => false, 'error' => __('Invalid API Key.')), 403);
		}

		// Sure it exists, but can they do anything with it?
		if ( ! method_exists($this, $controller_method))
		{
			$this->response(array('status' => false, 'error' => __('Unknown method.')), 404);
		}
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			{
				$this->response(array('status' => false, 'error' => __('This API key has reached the hourly limit for this method.')), 401);
			}
			...
			$authorized OR $this->response(array('status' => false, 'error' => __('This API key does not have enough permissions.')), 401);
		}
	...
	protected function _perform_ldap_auth($username = '', $password = NULL)
	{
		...
		if ($ldapconn) {
			...
			// verify binding
			if ($ldapbind) {
				log_message('debug', __('LDAP Auth: bind successful'));
			} else {
				log_message('error', __('LDAP Auth: bind unsuccessful'));
				return false;
			}
		...
		if (($res_id = ldap_search( $ldapconn, $ldapbasedn, "uid=$username")) == false) {
			log_message('error', __('LDAP Auth: User ') . $username . __(' not found in search'));
			return false;
		}

		if (ldap_count_entries($ldapconn, $res_id) != 1) {
			log_message('error', __('LDAP Auth: failure, username ') . $username . __('found more than once'));
			return false;
		}

		if (( $entry_id = ldap_first_entry($ldapconn, $res_id))== false) {
			log_message('error', __('LDAP Auth: failure, entry of searchresult could not be fetched'));
			return false;
		}

		if (( $user_dn = ldap_get_dn($ldapconn, $entry_id)) == false) {
			log_message('error', __('LDAP Auth: failure, user-dn could not be fetched'));
			return false;
		}

		// User found, could not authenticate as user
		if (($link_id = ldap_bind($ldapconn, $user_dn, $password)) == false) {
			log_message('error', __('LDAP Auth: failure, username/password did not match: ') . $user_dn);
			return false;
		}

		log_message('debug', __('LDAP Auth: Success ') . $user_dn . __(' authenticated successfully'));
	...
	protected function _check_login($username = '', $password = NULL)
	{
		...
		if ($auth_source == 'ldap')
		{
			log_message('debug', __('performing LDAP authentication for $username'));
	...
	protected function _check_whitelist_auth()
	{
		...
		{
			$this->response(array('status' => false, 'error' => __('Not authorized')), 401);
		}
	}
