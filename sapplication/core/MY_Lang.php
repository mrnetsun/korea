	...
	***************************************************/
	...
	function default_lang()
	{
		...
		$browser_lang = substr($browser_lang, 0,2);
 		...
 		return 'vn';
		//return (array_key_exists($browser_lang, $this->languages)) ? $browser_lang: 'vn';
	}
	...
	public function getModuleLang($_langfile, $path,$return=false){
			$lang = array();

			if($_langfile && file_exists($path.$_langfile.'_lang'.EXT)){
				include_once $path.$_langfile.'_lang'.EXT;
				$this->language = array_merge($this->language, $lang);
				$this->is_loaded[] = $_langfile.'_lang'.EXT;
			}
