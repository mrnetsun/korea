...
class ImageConverter {
	...
	function imageConverter() {
		...
		if($numargs > 3) $this->target = func_get_arg(3);
		
		/* ask the type of original file */
		$fileinfo  		= pathinfo($imagefile);
		$imtype 		= $fileinfo["extension"];
		if($numargs > 4) $this->imname = func_get_arg(4);
		else$this->imname 	= basename($fileinfo["basename"],".".$imtype);
			...
			switch($validtype){
				...
				case 'jpg' 	:
					if($this->imtype == 'gif' or $this->imtype == 'png') {
						...
						$image = $this->replaceTransparentWhite($this->im);
						imageJPEG($image,$this->target.$this->imname.".jpg");
					} else
					imageJPEG($this->im,$this->target.$this->imname.".jpg");
					break;
				case 'gif' :
					imageGIF($this->im,$this->target.$this->imname.".gif");
					break;
				case 'png' :
					imagePNG($this->im,$this->target.$this->imname.".png");
					break;
				case 'wbmp' :
					imageWBMP($this->im,$this->target.$this->imname.".wbmp");
					break;
				case 'swf' :
					$this->imageSWF($this->im,$this->target.$this->imname.".swf");
					break;
