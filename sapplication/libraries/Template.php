...
class Template
{
	...
	public $_theme = NULL;
	public $_theme_path = NULL;
	public $_layout = FALSE; // By default, dont wrap the view with anything
	public $_layout_subdir = ''; // Layouts and partials will exist in views/layouts
	...
	public $_parser_enabled = TRUE;
	public $_parser_body_enabled = TRUE;

	public $_theme_locations = array();
	...
	/**
	 * Put extra javascipt, css to footer
	 *
	 * @access	public
	 * @param	 string	$line	The line being added to head
	 * @return	void
	 */
	public function append_footerLoad($line){
		$this->_footer[] = $line;
		return $this;
	}
		...
		if ( ! empty($this->_theme))
		{
			...
			foreach ($this->_theme_locations as $location)
			{
				...
				foreach ($theme_views as $theme_view)
				{
					...
					{
						//debug(array_merge($this->_data ,$data),false);
	...
	private function _load_view($view, array $data, $parse_view = TRUE, $override_view_path = NULL)
	{
		...
		return $this->post_localize($content);
	}
	...
	private function post_localize($html = '')
	{
		// smarty open tag
		$ldq = preg_quote('{');
		...
		preg_match_all("/{$ldq}\s*({$cmd})\s*([^{$rdq}]*){$rdq}([^{$ldq}]*){$ldq}\/\\1{$rdq}/", $html, $matches);
		
		
		// No match
		if (!$matches || !count($matches)) return $html;
		...
		for ($i = 0; $i < count($matches[2]); $i++)
		{
			...
			$matches[3][$i] = __($this->fs($matches[3][$i]));
			...
			// Replace parameters
			$exploded = explode(' ', $matches[2][$i]);
			foreach ($exploded as $element)
			{
				$exp = explode('=', $element);
				if (count($exp))
				{
					$matches[3][$i] = str_replace('%'.$exp[0], trim($exp[1], '\'"'), $matches[3][$i]);
				}
			}
			$html = str_replace($matches[0][$i], $matches[3][$i], $html);
		}
