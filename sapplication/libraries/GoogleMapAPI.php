...
class GoogleMapAPI {
    ...
    //Lucdt
    /**
     *  add more customize code on onLoad function
     * @var string
     */
    var $_onloadCustomerCode = '';
    
    /**
     * Add custom code to callBack
     * @var string
     */
    var $callBackSearch = '';
    
    /**
     * allow drag & drop Maker
     * @var bool
     */
    var $maker_draggable = false;
 ...
 function addWaypoints($lat, $lon, $stopover = TRUE)
 {
  ...
  function addWaypointByAddress($address,$stopover = TRUE)
  {
	...
	*/
     ...
     * @param string $script_var
     * @return string
     */
    function getMapJS($script_var='') {
		$_script = "";
		$_key = $this->map_id;
        ...
        $_script .= "var polyOptions = {
        		...
        		var numberMaker = 0; var polys =[]; 
        		var infowindow = new google.maps.InfoWindow(); \n";
		$_script .="var geoCoder = null; \n";
        ...
        $_script.= " geoCoder = new google.maps.Geocoder(); geoCoder.firstItem = {};";
        ...
        //added Lucdt
        if($this->_onloadCustomerCode){
        	$_script.= $this->_onloadCustomerCode;
        }
        //end
    ...
    function getCreateMarkerJS() {
    	...
    	$_output.="};  
			    ...
			    var new_marker = new google.maps.Marker(marker_options);
			  	new_marker.setIcon('http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld='+title+'|FC8177|000000');
			    ...
			    if(html!=''){
			        ...
			        google.maps.event.addListener(new_marker, '".$this->window_trigger."', function() {
					 ...
					 google.maps.event.addListener(new_marker, 'dragend', function () {
	           				if(typeof(maker_dragend) ==='function'){
    							maker_dragend(new_marker);
    						}
	      			 });
    ...
    function getMap() {
    	$_output ='';
    	$buttonClose = ($this->buttonClose)?'<button type="button" class="button button-gray" onclick="parent.Common.closeOverlay()">'.__("BTN_CLOSE").'</button>':'';
    	if($this->local_search){
    		$_output.= '<div class="googleMapSearch form-inline" style="margin:10px 0px;">
    		    <div class="input">
			    <input placeholder="'.__("Search Address").'" onkeypress="'.$this->map_id.'_enterSearch(event)" id="'.$this->map_id.'_inputAddress" style="width:300px" type="text">
			    <button class="button button-blue" id="'.$this->map_id.'_submitSearch" type="button" onclick="'.$this->map_id.'_SearchAddress()">'.__("BTN_SEARCH").'</button> '.$buttonClose.'
			    </div>
    		</div>
    		<script>
    		function '.$this->map_id.'_enterSearch(event){    			
				 if (event.which == 13 || event.keyCode == 13) {
			            '.$this->map_id.'_SearchAddress();			            
			            event.preventDefault();
			            
			      }
			}
    		
    		function '.$this->map_id.'_SearchAddress(){
				 query = document.getElementById("'.$this->map_id.'_inputAddress").value;
				 if(geoCoder){
				 	geoCoder.geocode({address: query}, '.$this->map_id.'_geocodeResult);
				 }
			}
		
			function '.$this->map_id.'_geocodeResult(response, status){
				if (status == google.maps.GeocoderStatus.OK && response[0]) {
					geoCoder.firstItem = response[0];
					map'.$this->map_id.'.fitBounds(geoCoder.firstItem.geometry.viewport)
					
					';
					if($this->callBackSearch){ $_output .= $this->callBackSearch; }
					$_output.='
			...
			//var autocomplete = new google.maps.places.Autocomplete(document.getElementById("'.$this->map_id.'_inputAddress"), {});			
        ...
        if(strlen($this->width) > 0 && strlen($this->height) > 0) {
            $_output .= sprintf('document.write(\'<div id="%s" class="googleMap" style="width: %s; height: %s; position:relative;"><\/div>\');',$this->map_id,$this->width,$this->height) . "\n";
        } else {
            $_output .= sprintf('document.write(\'<div id="%s" class="googleMap" style="position:relative;"><\/div>\');',$this->map_id) . "\n";     
        }
        //$_output .= '}';
