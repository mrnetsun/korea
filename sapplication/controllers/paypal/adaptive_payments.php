...
class Adaptive_payments extends MY_Controller 
{
	...
	{
		$InvoiceID = uniqid('IID-');
		$TrackingID = uniqid('TID-');
		
		// Prepare request arrays
		$PayRequestFields = array(
								'ActionType' => 'PAY', 								// Required.  Whether the request pays the receiver or whether the request is set up to create a payment request, but not fulfill the payment until the ExecutePayment is called.  Values are:  PAY, CREATE, PAY_PRIMARY
								'CancelURL' => site_url('payment/pay_cancel'), 									// Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
								...
								'Pin' => '', 										// The sener's personal id number, which was specified when the sender signed up for the preapproval
								'PreapprovalKey' => '', 							// The key associated with a preapproval for this payment.  The preapproval is required if this is a preapproved payment.  
								'ReturnURL' => site_url('payment/pay_return'), 									// Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
								'ReverseAllParallelPaymentsOnError' => '', 			// Whether to reverse paralel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
								'SenderEmail' => 'lucdt@ideavietnam.com', 								// Sender's email address.  127 char max.
		...
		debug($Receivers,false);					
		$PayPalResult = $this->paypal_adaptive->Pay($PayPalRequestData);
		
		$this->session->set_userdata('testpaypal',$PayPalResult);
		...
		else
		{
			...
			echo anchor($PayPalResult['RedirectURL']);
			exit();
		}
	}
	...
	function Payment_details()
	{
		...
		$PayPalResult = $this->session->set_userdata('testpaypal');
		// Prepare request arrays
