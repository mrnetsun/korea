...
var Common = {
    ...
    deleteComment:function(id)
    {
        ...
        $('#delete'+id).click(function(){
            $.ajax(
                {
                    url:"http://korea.dev/vn/doing/comment/delete.html"+id,
                    type:"post",
	...
	alert:function(msg){
		try{
			...
			alertMe.open({title:"Thông báo ",width:300,content:'<div class="message"><div><h3 class="floatleft"><span class="toolbar_icon-16 toolbar_icon-16-info"></span></h3><p class="p-alert" style="padding-top:3px;">'+msg+'</p></div><br><div class="clear tc"><a class="btn" onclick="Common.closeOverlay()">OK</a></div></div>'});
	...
	error: function(msg){
		try{
			alertMe = Overlay.panel({title:true,center:true});
			alertMe.open({title:"Lỗi ",width:300,content:'<div class="message"><p  class="p-alert">'+msg+'</p></div><div class="clear tc"><a class="btn" onclick="Common.closeOverlay()">OK</a></div></div>'});
...
//			$closer.find('.modal').hide();
	...
	load : function(obj,url){
		$('#'+obj).html('<img src="'+STYLES_PATH+'images/load.gif" align="middle"/>'+' Đang tải...');
		$('#'+obj).load(url);
	},
	/**
		...
		try{
			...
			if(w_url.indexOf("delete")!=-1){
                ...
                if( setting.messageConfirm == '' || typeof setting.messageConfirm =='undefined')
                    setting.messageConfirm = 'Bạn thực sự muốn xóa?';
	...
	formSubmit:function(obj,form,task,wtitle,messageConfirm){
		...
		switch(task){
				case 'add':
					...
					addOverlay.open({title:'Thêm mới',url:w_url,data: $('#'+form).serialize()});

					break;
				case 'edit':
					var checked = Common.countChecked(form);
					if(!checked){
						Common.alert('Hãy chọn phần tử trong danh sách.');
						return false;
					}
					w_url = $(obj).attr('data-url');
                    if(!w_url) w_url = $(obj).attr('href');
					var addOverlay = Overlay.panel({'id':form + '_overlay_add'});
					addOverlay.open({title:'Sửa',url:w_url,data: $('#'+form).serialize()});
					break;
				case 'overlay':
	...
	openGoogleMap:function(input,latlong,address,preview){
		...
		try{
			...
			googleOverlay.open({title:"Google Map",width:750,height:530,iframe:true,url:"http://korea.dev/vn/common/googlemap.html"+"/"+input+'/'+latlong+'/'+address+'/'+preview});
	...
	frontGoogleMap:function(input,latlong,address,preview){
			...
			var url = "http://korea.dev/"+"common/googlemap/"+input+'/'+latlong+'/'+address+'/'+preview;
	...
	openGoogleMap2:function(input,latlong){
		...
		try{
			...
			googleOverlay.open({title:"Google Map",width:750,height:530,iframe:true,url:"http://korea.dev/vn/common/googlemap2.html"+"/"+input+'/'+latlong});
	...
	makeTranslation : function (obj,overlay){
		...
		if(!lang){
			Common.alert("Lỗi chưa chọn ngôn ngữ");
			return false;
		}
		...
		if(overlay){
			...
			addOverlay.open({title:'Translation',url:url});
		}else{
			Common.goUrl(url);
		}
	...
	imageBox:function(obj,large){
		...
		addOverlay.open({title:"Xem",content: img});
	},
	...
	getRegion: function(objCountry,region,city,empty){
		...
		if(country){		
			$.ajax({
				url: "http://korea.dev/vn/common/getStates.html",
				type: "POST",
				...
				success:function(data){
					try{
						...
						if(data.ok){
							...
							if(region){
								region.empty();
								if(empty) region.append('<option value="">--Lựa chọn--</option>');
							...
							if(city){
								city.empty();
								if(empty) city.append('<option value="">--Lựa chọn--</option>');
	...
	getCity: function(objRegion,objCountry,city,empty){
		...
		var city = $('#'+city);
		$.ajax({
			url: "http://korea.dev/vn/common/getCity.html",
			type: "POST",
	...
	importMemberRadio:function(obj)
    {
        $(obj).parent().find('input:checked').each(function(){
            if($(this).val()!='true'){
                ...
                $.ajax({
                    url: "http://korea.dev/vn/projects/insertMember.html",
                    type:"post",
                    data:formData,
                    success:function(data){
                        try{
                            ...
                            if(data.ok){
                                ...
                                var urlLoad ="http://korea.dev/vn/projects/loadMemberList.html";
	...
	unapprove: function(url,post){
		if(!confirm('Bạn muốn loại bỏ?')) return;
		$.ajax({
			url: url,
			type: "POST",
			data:"&cid=" +post,
	...
	replyMessage:function(id){
		Common.closeOverlay();
		w_url = 'http://korea.dev/vn/message/reply.html'+'/'+id;
		var addOverlay = Overlay.panel({'id':'reply_overlay_add'});
		addOverlay.open({title:'Reply',url:w_url,data:''});
	},
	...
	removeMyTranslation:function(obj,id){
		...
		var url ='http://korea.dev/vn/common/removeTrans.html/'+id;
		$.ajax({
			url: url,
			type: "POST",			
			success:function(data){
    ...
    loadShoutComment: function(data){
        try{
            var data = $.parseJSON(data);
            $('#'+data.code+'_shoutbox').load('http://korea.dev/vn/system/shoutbox/load.html/'+data.code);
        }catch (e){
            Common.jsonError(e);
        }
    },
	...
	sendMail:function(obj){
		...
		BootstrapDialog.show({
			...
			message: $('<div class="popupEmailEnquery"><img style="width: 20px; height: 20px;" src="'+ROOT_URL+'styles/images/loading.gif"/>Đang tải...</div>').load('http://korea.dev/vn/home/company/sendMailForm.html/?company_id='+company_id),
