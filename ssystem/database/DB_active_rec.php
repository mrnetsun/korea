...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function join($table, $cond, $type = '')
	{
		...
		if (preg_match('/([\w\.]+)([\W\s]+)(.+)/', $cond, $match))
		{
			...
			//Comment out by Lucdt 2013/06/13
			//$match[1] = $this->_protect_identifiers($match[1],FALSE);
			//$match[3] = $this->_protect_identifiers($match[3],FALSE);
		...
		foreach ($key as $k => $v)
		{
			...
			if ( ! is_null($v))
			{
				if ($escape === TRUE)
				{
					...
					$k = $this->_protect_identifiers($k, FALSE, $escape,TRUE,TRUE);
	...
	protected function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '')
	{
		...
		foreach ($field as $k => $v)
		{
			...
			//update by Lucdt			
			if(!strpos($k,'.'))	$k = $this->_protect_identifiers($k);
