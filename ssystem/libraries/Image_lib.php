...
class CI_Image_lib {
	...
	var $wm_opacity			= 80;			// Image opacity: 1 - 100  Only works with image
	...
	function getDimension($img,$d = 140){
		$output = array();
		if ($img_dim = getimagesize($img)) {
			...
			if ($thumb_width > $thumb_height) {
				$output['height'] = $d;
				$output['width'] = round($thumb_width *($d/$thumb_height));
			}else{
				$output['width'] = $d;
				$output['height'] = round($thumb_height *($d/$thumb_width));
			}
