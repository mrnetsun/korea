<?php
 //localTime
 //img_status($item->status,site_url('banner/campaign/dataSwitch/'.$item->campaign_id.'/status'));

$formBase = new FormBase();
echo $formBase->create('banner/campaign/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl w100">
                    	<div class="sortable <?php echo ($sort_key =='clicks')?$sort_dir:'';?>" onclick="Common.sort(this,'clicks')"><?php echo __("LBL_CLICK");?></div>                    	
                    </th>                    
					<th class="tl w100">
                    	<div class="sortable <?php echo ($sort_key =='impression')?$sort_dir:'';?>" onclick="Common.sort(this,'impression')"><?php echo __("LBL_IMPRESSION");?></div>                    	
                    </th>                    
					<th class="tl w100">
                    	<div class="sortable <?php echo ($sort_key =='days')?$sort_dir:'';?>" onclick="Common.sort(this,'days')"><?php echo __("LBL_NUMBER_DAY");?></div>
                    </th>                    
					<th class="tl w120">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->campaign_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
						<td class="tc"><?php echo $item->campaign_id;?></td>
					                    
					<td class="tl">
                    	<div><?php echo $item->title?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->clicks?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->impression?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->days?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->start_date?></div>                    	
                    </td>                    
					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('banner/campaign/dataSwitch/'.$item->campaign_id.'/status'))?></div>
                    </td>                    
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('banner/campaign/detail/'.$item->campaign_id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('banner/campaign/edit/'.$item->campaign_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('banner/campaign/delete/'.$item->campaign_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
