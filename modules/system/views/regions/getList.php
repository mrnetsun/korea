...
echo $formBase->create('system/region/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl span2">
                    	<div class="sortable <?php echo ($sort_key =='region_code')?$sort_dir:'';?>" onclick="Common.sort(this,'region_code')"><?php echo __("Region code");?></div>                    	
                    </th>                    
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='region_name')?$sort_dir:'';?>" onclick="Common.sort(this,'region_name')"><?php echo __("Region name");?></div>                    	
                    </th>                    
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='country_code')?$sort_dir:'';?>" onclick="Common.sort(this,'country_code')"><?php echo __("Country");?></div>                    	
                    </th>
                    <th class="tl span1">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->region_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
                    
					                    
					<td class="tl">
                    	<div>
                    	<a title='<?php echo __('LBL_EDIT');?>' onclick="return Common.linkSetup(this,{ajax:true,overlay:true,title:'<?php echo __('LBL_EDIT')?>'})" href="<?php echo site_url('system/region/edit/'.$item->region_id)?>">
                    	<?php echo $item->region_code?>
                    	</a>
                    	</div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->region_name?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->nicename?></div>                    	
                    </td>
                    <td class="tc">
                    	<div><?php echo img_status($item->status,site_url('system/region/dataSwitch/'.$item->region_id.'/status'));?></div>                  	
                    </td>
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="settings-button">
                    		...
                    		<div class="toolbar-icons" style="display: none;">							
							    <?php echo ajax_link('system/region/detail/'.$item->region_id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                                <?php echo ajax_link('system/region/edit/'.$item->region_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                                <?php echo ajax_link('system/region/delete/'.$item->region_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
						</div>
                    	</div>
