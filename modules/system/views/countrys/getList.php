...
echo $formBase->create('system/country/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl span1">
                    	<div class="sortable <?php echo ($sort_key =='iso')?$sort_dir:'';?>" onclick="Common.sort(this,'iso')"><?php echo __("Code");?></div>                    	
                    </th>                    
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='nicename')?$sort_dir:'';?>" onclick="Common.sort(this,'nicename')"><?php echo __("Country name");?></div>                    	
                    </th>                    
					<th class="tl sapn1">
                    	<div class="sortable <?php echo ($sort_key =='phonecode')?$sort_dir:'';?>" onclick="Common.sort(this,'phonecode')"><?php echo __("Phone code");?></div>                    	
                    </th>
                    <th class="tl span1">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
					...
					<td class="tl">
                    	<div><?php echo $item->iso?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div>
                    	<a title='<?php echo __('LBL_EDIT');?>' onclick="return Common.linkSetup(this,{ajax:true,overlay:true,title:'<?php echo __('LBL_EDIT')?>'})" href="<?php echo site_url('system/country/edit/'.$item->id)?>">
                    	<?php echo $item->nicename?>
                    	</a>
                    	</div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->phonecode?></div>                    	
                    </td>
					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('system/country/dataSwitch/'.$item->id.'/status'));?></div>                  	
                    </td>
                    <td class="tc last ">
                    	...
                    	<div class="settings-button">
                            ...
                            <?php echo ajax_link('system/country/detail/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('system/country/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('system/country/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>				
