...
echo $formBase->create('system/slider/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='name')?$sort_dir:'';?>" onclick="Common.sort(this,'name')"><?php echo __("Name");?></div>                    	
                    </th>                   
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->slide_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
                    
					                    
					<td class="tl">
						<div class="w50 h80 left">
						<?php echo base_img($item->image,array('height'=>40,'width'=>40))?>
						</div>
                    	<div>
                    	<div class="bold">
                    	<a title='<?php echo __('LBL_EDIT');?>' onclick="return Common.linkSetup(this,{ajax:true,overlay:true,title:'<?php echo __('LBL_EDIT')?>'})" href="<?php echo site_url('system/slider/edit/'.$item->slide_id)?>">
                    	<?php echo $item->name?>
                    	</a>
                    	</div>
                    	<div class="note"><?php echo $item->url?></div>
                    	<div><?php echo $item->descriptions?></div>
                    	</div>                   	
                    </td>                    
					                   
					<td class="tl">
                    	<div><?php echo img_status($item->status,site_url('system/slider/dataSwitch/'.$item->slide_id.'/status'));?></div>                    	
                    </td>
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="settings-button">
                    		...
                    		<div class="toolbar-icons" style="display: none;">							
							    <?php echo ajax_link('system/slider/detail/'.$item->slide_id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                                <?php echo ajax_link('system/slider/edit/'.$item->slide_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                                <?php echo ajax_link('system/slider/delete/'.$item->slide_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
						</div>
                    	</div>
