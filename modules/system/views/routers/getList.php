<?php
 //localTime
 //img_status($item->status,site_url('system/router/dataSwitch/'.$item->id.'/status'));

$formBase = new FormBase();
echo $formBase->create('system/router/saveAll');
	...
	<table>
		<thead><!-- universal table heading -->
			<tr>
				...
				<th class="tl w20">
					<div class="sortable <?php echo ($sort_key =='id')?$sort_dir:'';?>" onclick="Common.sort(this,'id')"><?php echo __("LBL_ID");?></div>
				</th>
				<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='slug')?$sort_dir:'';?>" onclick="Common.sort(this,'slug')"><?php echo __("LBL_ALIAS");?></div>                    	
                    </th>                    
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='controller')?$sort_dir:'';?>" onclick="Common.sort(this,'controller')"><?php echo __("LBL_URI");?></div>                    	
                    </th>
				<th class="tc w100">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
						...
						<td class="tl">
                    	<div><?php echo $item->slug?></div>                    	
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->controller?></div>                    	
                    </td>
						<td class="tc">
							<div><?php echo $item->position;?></div>
						</td>
						<td class="tc">
							<div><?php echo img_status($item->status,site_url('system/router/dataSwitch/'.$item->id.'/status'));?></div>
						</td>
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons" >
                            <?php echo ajax_link('system/router/detail/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('system/router/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('system/router/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
