...
echo $formBase->create('system/partner/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='partnerName')?$sort_dir:'';?>" onclick="Common.sort(this,'partnerName')"><?php echo __("Name");?></div>                    	
                    </th>                    
					<th class="tl span1 tc">
                    	<div class="sortable <?php echo ($sort_key =='status')?$sort_dir:'';?>" onclick="Common.sort(this,'status')"><?php echo __("LBL_STATUS");?></div>
                    </th>
                    <th class="tl span2 tc">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->partnerId,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
                    
					                    
					<td class="tl">
                    	<div>
                    	<div class="left mg5"><?php echo base_img($item->logo,array('width'=>50,'height'=>'50'))?></div>
                    	<div class="bold"><a title='<?php echo __('LBL_EDIT');?>' onclick="return Common.linkSetup(this,{ajax:true,overlay:true,title:'<?php echo __('LBL_EDIT')?>'})" href="<?php echo site_url('system/partner/edit/'.$item->partnerId)?>"><?php echo $item->partnerName?></a></div>
                    	<div class="note"><?php echo $item->website?></div></div>                    	
                    </td>                    
					<td class="tl tc">
                    	<div><?php echo img_status($item->status,site_url('system/partner/dataSwitch/'.$item->partnerId.'/status'));?></div>                    	
                    </td>
					<td class="tl tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="settings-button">
                    		...
                    		<div class="toolbar-icons" style="display: none;">							
							    <?php echo ajax_link('system/partner/detail/'.$item->partnerId,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                                <?php echo ajax_link('system/partner/edit/'.$item->partnerId,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                                <?php echo ajax_link('system/partner/delete/'.$item->partnerId,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
                            </div>
                    	</div>
