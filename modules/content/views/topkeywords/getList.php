<?php
 //localTime
 //img_status($item->status,site_url('content/topkeyword/dataSwitch/'.$item->id.'/status'));

$formBase = new FormBase();
echo $formBase->create('content/topkeyword/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='keyword')?$sort_dir:'';?>" onclick="Common.sort(this,'keyword')"><?php echo __("LBL_KEYWORD");?></div>                    	
                    </th>                    
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='url')?$sort_dir:'';?>" onclick="Common.sort(this,'url')"><?php echo __("LBL_URL");?></div>                    	
                    </th>                    
					<th class="tc span1">
                    	<div class="sortable <?php echo ($sort_key =='target')?$sort_dir:'';?>" onclick="Common.sort(this,'target')"><?php echo __("LBL_TARGET");?></div>                    	
                    </th>
				<th class="tc span1">
					<div class="sortable <?php echo ($sort_key =='position')?$sort_dir:'';?>" onclick="Common.sort(this,'position')"><?php echo __("LBL_POSITION");?></div>
				</th>
				<th class="tc w100">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
					...
					<td class="tc">
                    	<div><?php echo $item->target?></div>                    	
                    </td>
						<td class="tc">
							<div><?php echo $item->position?></div>
						</td>
						<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('content/topkeyword/dataSwitch/'.$item->id.'/status'))?></div>
                    </td>                    
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('content/topkeyword/detail/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('content/topkeyword/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('content/topkeyword/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
