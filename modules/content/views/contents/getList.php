<?php
 //localTime
 //img_status($item->status,site_url('content/content/dataSwitch/'.$item->content_id.'/status'));

$formBase = new FormBase();
echo $formBase->create('content/content/saveAll');
	...
	<table>
		<thead><!-- universal table heading -->
			<tr>
				...
				<th class="tc w20">
					<div class="sortable <?php echo ($sort_key =='content_id')?$sort_dir:'';?>" onclick="Common.sort(this,'content_id')"><?php echo __("LBL_ID");?></div>
				</th>
					<th class="tl">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->content_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
						<td class="tc">
							<div><?php echo $item->content_id?></div>
						</td>
					                    
					<td class="tl">
                    	<div class="bold"><?php echo $item->title?></div>
						<div><?php echo word_limiter($item->detail,50)?></div>
					</td>

					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('content/content/dataSwitch/'.$item->content_id.'/status'))?></div>
                    </td>                    
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo anchor('home/home/page/'.$item->slug,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW','target'=>'_blank'));?>
                            <?php echo anchor('content/content/edit/'.$item->content_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('content/content/delete/'.$item->content_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
