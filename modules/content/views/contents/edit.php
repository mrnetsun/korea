<?php 
	...
	echo $formBase->create('content/content/update',array('ajax'=>_getBaseName('List'),'ckeditor'=>1,'class'=>'form-horizontal'));
	$formBase->fillData($fillData);
	echo $formBase->hidden('content_id');
?>
    <div class="screenView span10">
        <div style="height:15px;"><h3></h3></div>
        <div class="section ">
			<?php echo $formBase->input('title',array('id'=>'titleId','label'=>"LBL_TITLE",'rules'=>"required",'class'=>"span8"))?>
			<?php echo $formBase->input('slug',array('id'=>'slugId','label'=>"LBL_SLUG",'rules'=>"",'class'=>"span8"))?>
			<?php echo $formBase->ckeditor('detail',array('label'=>"LBL_CONTENT",'rules'=>"required",'class'=>"span8 h100"))?>
			<?php echo $formBase->status('status',array('label'=>"LBL_STATUS",'value'=>1))?>
