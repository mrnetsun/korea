<?php
 //localTime
 //img_status($item->status,site_url('content/feature/dataSwitch/'.$item->id.'/status'));

$formBase = new FormBase();
echo $formBase->create('content/feature/saveAll');
	...
	<table>
		<thead><!-- universal table heading -->
			<tr>
                ...
                <th class="tl span2">
                    <div class="sortable <?php echo ($sort_key =='company_id')?$sort_dir:'';?>" onclick="Common.sort(this,'company_id')"><?php echo __("LBL_COMPANY_ID");?></div>
                </th>
                <th class="tl">
                    <div class="sortable <?php echo ($sort_key =='company_name')?$sort_dir:'';?>" onclick="Common.sort(this,'company_name')"><?php echo __("LBL_COMPANY_NAME");?></div>
                </th>
                <th class="tc w100">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tl">
                        <?php
                            if($item->end_date && $item->end_date < date("Y-m-d")){
                                $badge = 'badge-inverse';
                            }

                            if($item->start_date && $item->start_date > date("Y-m-d")){
                                $badge = 'badge-important';
                            }

                            if($item->start_date && $item->start_date < date("Y-m-d") && date("Y-m-d") < $item->end_date){


                                $start  =  new DateTime($item->end_date);
                                $now    = new DateTime();
                                $diff = $start->diff($now);

                                if($diff->days <=2)  $badge = 'badge-warning';
                                else  $badge = 'badge-success';
                            }
                        ?>
                        <div class="bold"><span class="badge   <?php echo $badge?>"><?php echo $item->company_id?></span></div>
                    </td>
					<td class="tl">
                        <div><?php echo $item->company_name?></div>
                    </td>
                        <td class="tc">
                            <div><?php echo ($item->start_date)?localDate($item->start_date):__("LBL_UNLIMITED")?></div>
                        </td>
                        <td class="tc">
                            <div><?php echo ($item->end_date)?localDate($item->end_date):__("LBL_UNLIMITED")?></div>
                        </td>
                        <td class="tc">
                            <div><?php echo img_status($item->status,site_url('content/feature/dataSwitch/'.$item->id.'/status'))?></div>
                        </td>
                        <td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('content/feature/detail/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('content/feature/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('content/feature/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
