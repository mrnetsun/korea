<?php
 //localTime
 //img_status($item->status,site_url('cronjob/news/dataSwitch/'.$item->id.'/status'));

$formBase = new FormBase();
echo $formBase->create('cronjob/news/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl w80">
                    	<div class="sortable <?php echo ($sort_key =='approve')?$sort_dir:'';?>" onclick="Common.sort(this,'approve')"><?php echo __("LBL_MODE");?></div>                    	
                    </th>                    
					<th class="tl span2">
                    	<div class="sortable <?php echo ($sort_key =='cateId')?$sort_dir:'';?>" onclick="Common.sort(this,'cateId')"><?php echo __("LBL_CATEGORY");?></div>                    	
                    </th>                    
					<th class="tl w80">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
					...
					<td class="tl">
                    	<div><?php echo $item->title?> <span class="label"><?php echo localDate($item->last_running)?></span></div>
						<div class="note"><?php echo $item->link?></div>
					</td>
					...
					<td class="tc">
                    	<div><?php echo ($item->approve)? __("LBL_AUTO"):__("LBL_MANUAL")?></div>
                    </td>                    
					<td class="tl">
                    	<div><?php echo $item->cate_name?></div>
                    </td>                    
					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('cronjob/news/dataSwitch/'.$item->id.'/status'))?></div>
                    </td>                    
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo anchor('cronjob/vneconomy/index/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('cronjob/news/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('cronjob/news/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
