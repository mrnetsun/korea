<?php
 //localTime
 //img_status($item->status,site_url('exam/testcategory/dataSwitch/'.$item->cate_test_id.'/status'));

$formBase = new FormBase();
echo $formBase->create('exam/testcategory/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl w50">
                    	<div class="sortable <?php echo ($sort_key =='cate_test_id')?$sort_dir:'';?>" onclick="Common.sort(this,'cate_test_id')"><?php echo __("LBL_ID");?></div>                    	
                    </th>                    
					<th class="tl">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->cate_test_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
                    
					                    
					<td class="tl">
                    	<div><?php echo $item->cate_test_id?></div>                    	
                    </td>                    
					<td class="tl">
					...
					<td class="tc ">
                    	<div><?php echo img_status($item->status,site_url('exam/testcategory/dataSwitch/'.$item->cate_test_id.'/status'))?></div>
                    </td>                    
					<td class="tc ">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('exam/testcategory/detail/'.$item->cate_test_id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('exam/testcategory/edit/'.$item->cate_test_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('exam/testcategory/delete/'.$item->cate_test_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
