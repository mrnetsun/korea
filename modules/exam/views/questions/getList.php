<?php
 //localTime
 //img_status($item->status,site_url('exam/question/dataSwitch/'.$item->question_id.'/status'));

$formBase = new FormBase();
echo $formBase->create('exam/question/saveAll');
	...
	<table>
		<thead><!-- universal table heading -->
			<tr>
				...
				<th class="tl w30">
					<div class="sortable <?php echo ($sort_key =='question_id')?$sort_dir:'';?>" onclick="Common.sort(this,'question_id')"><?php echo __("LBL_ID");?></div>
				</th>
				<th class="tl">
					<div class="sortable <?php echo ($sort_key =='title')?$sort_dir:'';?>" onclick="Common.sort(this,'title')"><?php echo __("LBL_TITLE");?></div>
				</th>
				<th class="tl span2">
					<div class="sortable <?php echo ($sort_key =='question_type')?$sort_dir:'';?>" onclick="Common.sort(this,'question_type')"><?php echo __("LBL_TYPE");?></div>
				</th>
					<th class="tl">
				...
				<th class="tc span1">
					<div class="sortable <?php echo ($sort_key =='point')?$sort_dir:'';?>" onclick="Common.sort(this,'point')"><?php echo __("LBL_POINT");?></div>
				</th>

				<th class="tl span2 w100">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->question_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
						<td class="tl">
							<div><?php echo $item->question_id?></div>
						</td>
					...
					<td class="tl">
						...
						<td class="tl">
							<div><?php echo $questionType[$item->question_type]?></div>
						</td>
						<td class="tl">
					...
					<td class="tc">
                    	<div><?php echo $item->point?></div>                    	
                    </td>                    

						<td class="tc">
							<div><?php echo img_status($item->status,site_url('exam/question/dataSwitch/'.$item->question_id.'/status'))?></div>
						</td>
						<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo anchor('exam/question/detail/'.$item->question_id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo anchor('exam/question/edit/'.$item->question_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('exam/question/delete/'.$item->question_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
