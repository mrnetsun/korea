<?php
 //localTime
 //img_status($item->status,site_url('exam/examcategory/dataSwitch/'.$item->exam_category_id.'/status'));

$formBase = new FormBase();
echo $formBase->create('exam/examcategory/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
                <td class="tc first w20"><input type="checkbox" value="true" class="checkAll" onclick="Common.checkAll(this)"></td>                
					...
					<th class="tl w30">
                    	<div class="sortable <?php echo ($sort_key =='exam_category_id')?$sort_dir:'';?>" onclick="Common.sort(this,'exam_category_id')"><?php echo __("LBL_ID");?></div>
                    </th>
				<th class="tl">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
                    ...
                    <td class="tc first">
                    	<?php echo $formBase->checkbox('cid[]',array('value'=>$item->exam_category_id,'onclick'=>"Common.isCheckAll(this)"));?>
                    </td>
                    <td class="tc"><div><?php echo $item->exam_category_id?></div></td>
					...
					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('exam/examcategory/dataSwitch/'.$item->exam_category_id.'/status'))?></div>
                    </td>
						<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('exam/examcategory/detail/'.$item->exam_category_id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('exam/examcategory/edit/'.$item->exam_category_id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('exam/examcategory/delete/'.$item->exam_category_id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
