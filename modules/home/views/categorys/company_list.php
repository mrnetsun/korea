<div id="companyList">
    <?php
    $refine = $this->session->userdata('Category.Refine');
    $strRefine = array();
    ...
    <div id="refineWrap" class="row">
        ...
        <form id="frmRefine" style="display: none;">
            ...
            foreach($facets as $group_key=>$group){
                if($group) {
                    echo '<ul class="list-unstyled refineGroup col-md-6">';
    ...
    <div class=" banner_search1">
        <?php echo page_banner('COM1')?>
    </div>
    <div class="gap-mini"></div>
    <ul class="booking-list">
        <?php

        if($results['response']['docs'])foreach($results['response']['docs'] as $k=>$item){
//        debug($item);
            ?>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.companySort a.sortItem').click(function(){
            homeController.sortByCategory(this);
        });

        $('#frmRefine :input[type=checkbox]').change(function(){
            homeController.refineCategory();
        });

        $('.moreRefine').click(function(){
            homeController.getMoreRefineCategory(this);
        });
