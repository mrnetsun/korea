...
class Practice extends Front_Controller {
    ...
    public function index(){
            ...
            //lay chuyen de
            $categories = $this->model->find_where('list',array('status'=>ACTIVE,'exam_category_id <>'=>4),array('from'=>'exam_category'));

            $this->template->set('categories',$categories);
            $this->view('practice/index');


    }

    public function chuyende($id='',$start=0){
        $limit = 30; $id = intval($id);

        if($start<=0) $start = 1;
        $offset = ($start-1)*$limit;

        $cate = $this->model->get($id,array('from'=>'exam_category'));
        $this->template->set('cate',$cate);

        //lay noi chuyen de
        $where = array('status'=>3,'cate_id'=> $id);
        $exams = $this->model->find_where('list',
            $where,
            array('from'=>'exam','limit'=>$limit,'offset'=>$offset,'order_by'=>array('exam_id'=>'DESC')));

        foreach($exams as $k=>$e){
            $exams[$k]->total_attempt = $this->mongo_db->where(array('exam_id'=>intval($e->exam_id)))
                ->count('practice_member');
        ...
        $total = $this->model->count_where($where,array('from'=>'exam'));


        $config['div'] = 'examList';
        $config['anchor'] = 'captchaAnchor';
        $config['base_url'] = '/luyen-thi/chuyende/'.$id;
    ...
    public function xem($id=''){
        ...
        if($this->LoginData){
            $myHistory = $this->mongo_db->select(
                ...
                ->order_by(array('start_time','DESC'))
                ->limit(10)
                ->get_where('practice_member',array('exam_id'=>$id,'memberId'=>$this->LoginData->memberId));
        ...
        $topMember = $this->mongo_db->aggregate('practice_member',$groups_q);

        $this->template->set('topMember',$topMember);

        $this->view('practice/xem',array('exam'=> $exam));
    }
    ...
    public function thi($id=''){
        ...
        //reset all
//        $this->mongo_db->delete_all('practice_member');

        $memberId = ($this->LoginData->memberId)? $this->LoginData->memberId:session_id();

        //get exam_status
        $exam_status = $this->mongo_db->get_where('practice_member',array('status'=>EXAM_DOING,'memberId'=>$memberId,'exam_id'=>$id));
        ...
        if(empty($exam_status)){
            ...
            $this->mongo_db->insert('practice_member',$exam_status);
        ...
        $this->view('practice/thi',array('exam'=> $exam),'','exam');
    }
    ...
    public function updateQuestion(){
        ...
        $anserSheet =  $this->mongo_db->where(array('_id'=>$_id))->find_one('practice_member');
...
//        }
    ...
    public function finish($sheet_id){
        ...
        $this->view('practice/anser_sheet',array('anserSheet'=>$anserSheet));
    }
    ...
    public function ketqua($sheet_id){
        ...
        $this->view('practice/quick_result',array('anserSheet'=>$anserSheet));
    }
    ...
    $this->view('practice/detail_result',array('anserSheet'=>$anserSheet));
    }
}
