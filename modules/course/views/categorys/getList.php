<?php
 //localTime
 //img_status($item->status,site_url('course/category/dataSwitch/'.$item->id.'/status'));

$formBase = new FormBase();
echo $formBase->create('course/category/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
				...
				<th class="tc w50">
					...
					<th class="tl">
                    	<div class="sortable <?php echo ($sort_key =='cate_name')?$sort_dir:'';?>" onclick="Common.sort(this,'cate_name')"><?php echo __("LBL_TITLE");?></div>                    	
                    </th>                    

					<th class="tl span3">
                    	<div class="sortable <?php echo ($sort_key =='parent_id')?$sort_dir:'';?>" onclick="Common.sort(this,'parent_id')"><?php echo __("LBL_ROOT");?></div>                    	
                    </th>                    
					<th class="tc w100">
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
					...
					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('course/category/dataSwitch/'.$item->id.'/status'))?></div>
                    </td>                    
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('course/category/detail/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('course/category/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('course/category/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
