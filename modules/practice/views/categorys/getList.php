<?php
 //localTime
 //img_status($item->status,site_url('practice/category/dataSwitch/'.$item->id.'/status'));

$formBase = new FormBase();
echo $formBase->create('practice/category/saveAll');
		...
		<thead><!-- universal table heading -->
			<tr>
				...
				<th class="tl">
					...
					<th class="tl w120">
                    	<div class="sortable <?php echo ($sort_key =='createdTime')?$sort_dir:'';?>" onclick="Common.sort(this,'createdTime')"><?php echo __("LBL_UPDATED");?></div>                    	
                    </th>               
            ...
            <?php if(!empty($aryData)){
            	...
            	foreach ($aryData as $item){            		
					...
					<td class="tl">
                    	<div><?php echo $item->parent_id?></div>                    	
                    </td>                    
					<td class="tc">
                    	<div><?php echo img_status($item->status,site_url('practice/category/dataSwitch/'.$item->id.'/status'))?></div>
                    </td>                    
					<td class="tc">
                    ...
                    <td class="tc last ">
                    	...
                    	<div class="toolbar-icons">
                            <?php echo ajax_link('practice/category/detail/'.$item->id,'<i class="action-icon-eye-open"></i>',array('overlay'=>'LBL_VIEW'));?>
                            <?php echo ajax_link('practice/category/edit/'.$item->id,'<i class="action-icon-edit"></i>',array('overlay'=>'LBL_EDIT'));?>
                            <?php echo ajax_link('practice/category/delete/'.$item->id,'<i class="action-icon-trash"></i>',array('callBack'=>'reload'));?>
